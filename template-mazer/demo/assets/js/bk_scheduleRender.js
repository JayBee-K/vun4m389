...
${data_hours == 23 ? '<tr data-date-time="24:00"></tr>' : ''}`
	...
	addCustomer('18:00', '18:30', '18:30', 2, {
		color: '#3a87ad',
		mess: true,
		cCount: 4,
		isProcess: true,
		extra: 15,
		classCus: ['cus-02', 'paid', 'free-axis', 'sv-1']
	});
	...
	addCustomer('22:15', '23:00', '23:00', 6, {
		color: '#8FD400',
		cCount: 2,
		isProcess: true,
		isNew: true,
		classCus: ['cus-03', 'booking', 'free-axis', 'sv-3']
	});
	...
	addCustomer('22:00', '22:30', '22:30', 4, {
		color: '#FE0883',
		isNew: true,
		classCus: ['cus-04', 'booking', 'free-axis', 'sv-2']
	});
	...
	addCustomer('18:00', '18:45', '19:45', 10, {
		color: '#910101',
		isNew: true,
		classCus: ['cus-06', 'booking', 'sv-5']
	});
	addCustomer('19:45', '21:30', '22:30', 11, {
		color: '#910101',
		isNew: true,
		classCus: ['cus-06', 'booking', 'sv-4']
	});
	addCustomer('22:30', '23:00', '23:00', 12, {
		color: '#910101',
		isNew: true,
		classCus: ['cus-06', 'booking', 'free-axis', 'sv-2']
	});
...
export function positionYByTime(time = new Date()) {
...
export function addCustomer(timeStar = "", timeEnd = "", tierTimeEnd = "", operatory = 0, opt = {
	color: '#c0c0c0',
	isNew: false,
	isProcess: false,
	mess: false,
    ...
    <div class="sch-event ${opt.classCus.map(cl => `${cl}`).join(' ')}" style="top:${topPosY}px;height:${bottomPosY - topPosY}px">
        ...
        <div class="actions"><a class="bi bi-trash handleRemoveSch"></a><a class="bi bi-gear handleOpenEditMenu"></a></div> 
        ${opt.isProcess ? '<span class="process"></span>' : ''}
        <a href="javascript:;" class="power-off handleRemoveSsAllEvent" data-status="12121"><i class="bi bi-power"></i></a>
        ${ipStoreTime}
    </div>
    `;
...
export function handleRemoveSch() {
	let buttonElm = document.querySelectorAll('.handleRemoveSch');
...
export function handleOpenEditMenu() {
	let buttonElm = document.querySelectorAll('.handleOpenEditMenu');
...
export function addBlankTime(timeStar = '', timeEnd = '', operatory = 0) {
	let contCell = $(eventStore).find('td:not(.time)').eq(operatory + 1);
	let contEle = contCell.children('.events');
...
export function OpenEditMenu(obj) {
...
export function freeAxisOp(obj) {
