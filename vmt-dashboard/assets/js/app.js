 ...
 * Handle Hàng Hóa
 */
export function handleAddGoods() {
	let handleGoods = document.getElementById('handleGoods');
	let btnAddGoods = document.getElementById('btnAddGoods');
											...
											<div class="goods-item_name">
												<div class="dashboard-form_item">
													<input type="text" placeholder="Tên hàng hóa"
													       value="电脑支架笔记本电脑支架 铝合金散热支架增高收纳懒"
											...
											<div class="goods-item_quantity">
												<div class="dashboard-form_item">
													<div class="dashboard-form_quantity">
														<button type="button"
														        class="dashboard-form_quantity__button" disabled>
...
export function handleRemoveGoods() {
	let handleGoods = document.getElementById('handleGoods');

	const btnRemoveGoods = document.querySelectorAll('.buttonRemoveGood');

	if (handleGoods !== null && btnRemoveGoods.length > 0) {
		btnRemoveGoods.forEach(function (button) {
			button.addEventListener('click', function () {
				button.closest('.goodsItem').remove();
			})
		})
	}
}
